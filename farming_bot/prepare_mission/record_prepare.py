"""录制任务准备操作 — 记录任务开始后的所有按键 + 摇杆输入.

使用方式:
  python -m farming_bot.prepare_mission.record_prepare

流程:
  1. 倒计时 10 秒 → 切到游戏 (任务已加载完成)
  2. 执行准备操作 (跳过对话、吃饭、准备装备等)
  3. 完成后长按 B (0.8秒) 停止录制
  4. 保存按键序列
"""
...
def record():
    ...
    print("操作说明:")
    print("  - 任务加载完成后开始录制")
    print("  - 录制所有准备操作 (跳过对话、吃饭、准备装备等)")
    print("  - 准备完成后长按 B (0.8秒) 停止录制")
    ...
    # 保存
    prepare_path = os.path.join(_OUTPUT_DIR, "prepare_sequence.json")
    ...
    print(f"保存: {prepare_path}")

    # 截屏保存最后一帧
    screenshot = sct.grab(monitor)
    frame = np.array(screenshot)
    frame = cv2.cvtColor(frame, cv2.COLOR_BGRA2BGR)
    last_frame_path = os.path.join(_OUTPUT_DIR, "prepare_done_frame.png")
    cv2.imwrite(last_frame_path, frame)
    print(f"完成截图: {last_frame_path}")
