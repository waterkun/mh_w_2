...
def record():
    ...
    print("操作说明:")
    print("  - 录制传送回营地的完整操作")
    print("  - 包括: 打开地图/菜单 → 选营地 → 确认传送")
    print("  - 等传送完成后按 长按B 停止录制")
    ...
    beep(1200, 500)
    print()
    print(">>> 录制开始! 执行传送回营地操作，完成后长按 B 停止 <<<")
    print()
    ...
    # 上一帧的按钮状态 (用于检测 press/release)
    prev_buttons = {}
    prev_triggers = {"LT": 0, "RT": 0}

    # B 键长按检测
    b_hold_start = None
    B_HOLD_THRESHOLD = 0.8  # 长按 0.8 秒停止
    ...
    # 保存
    reset_path = os.path.join(_OUTPUT_DIR, "reset_sequence.json")
    ...
    print(f"保存: {reset_path}")

    # 截屏保存最后一帧
    screenshot = sct.grab(monitor)
    frame = np.array(screenshot)
    frame = cv2.cvtColor(frame, cv2.COLOR_BGRA2BGR)
    last_frame_path = os.path.join(_OUTPUT_DIR, "reset_arrival_frame.png")
